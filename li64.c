...
static inline uint64_t divu3(uint64_t n)
{
    uint64_t x = n;
    x = (x << 2) + x + 5;
    x = (x >> 4) + x;
...
static inline uint64_t divu5(uint64_t n)
{
    uint64_t x = n;
    x = (x << 1) + x + 3;
    x = (x >> 4) + x;
...
static inline uint64_t divu7(uint64_t n)
{
    uint64_t x = n + 1;
#if LI
    x = (x << 2) + (x >> 1);
...
static inline uint64_t divu9(uint64_t n)
{
    uint64_t x = n;
    x = x + 1;
    x = (x << 1) + x + (x >> 1);
...
static inline uint64_t divu11(uint64_t n)
{
    uint64_t x = n, y;
    x = x + 1;
    y = (x << 2) + x;
...
static inline uint64_t divu13(uint64_t n)
{
    uint64_t x = n;
    x = x + 1;
    x = (x << 2) + (x >> 1);
...
static inline uint64_t divu15(uint64_t n)
{
    uint64_t x = n;
    x = x + 1;
    x = (x << 2) + (x >> 2);
...
static inline uint64_t divu17(uint64_t n)
{
    uint64_t x = n;
    x = (x << 2) + x + 5;
    x = (x >> 1) + x;
...
static inline uint64_t divu19(uint64_t n)
{
    uint64_t x = n;
    x = (x << 1) + x + 2;
    x = (x >> 3) + x;
...
static inline uint64_t divu21(uint64_t n)
{
    uint64_t x = n;
    x = (x << 1) + x + 3;
    x = (x >> 6) + x;
...
static inline uint64_t divu23(uint64_t n)
{
    uint64_t x = n;
    x = x + 1;
    x = (((x >> 3) + x) >> 1) + x + (x << 2);
...
static inline uint64_t divu25(uint64_t n)
{
    uint64_t x = n, y;
    y = x + 1;
    x = (y << 2) + x + (y >> 3);
...
static inline uint64_t divu27(uint64_t n)
{
    uint64_t x = n, y;
#if LI
    y = (x << 1) + x + 15;
...
static inline uint64_t divu29(uint64_t n)
{
    uint64_t x = n, y;
    y = (x >> 2) + (x << 2) + 3;
    y = (y >> 5) + y;
...
static inline uint64_t divu31(uint64_t n)
{
    uint64_t x = n;
    x = x + 1;
    x = (x << 2) + (x >> 3);
...
static inline uint64_t divu33(uint64_t n)
{
    uint64_t x = n, y;
    x = x + 1;
    y = (x << 1) + x;
...
static inline uint64_t divu35(uint64_t n)
{
    uint64_t x = n, y;
    x = x + 1;
    y = (x << 1) + x;
...
static inline uint64_t divu37(uint64_t n)
{
    uint64_t x = n, y;
    x = (x << 2) + x + 4;
    y = (x >> 3) + x;
...
static inline uint64_t divu39(uint64_t n)
{
    uint64_t x = n + 1, y;
#if LI
    y = (x << 1) + x;
...
static inline uint64_t divu41(uint64_t n)
{
    uint64_t x = n, y;
    y = (x << 1) + x + 2;
    x = (x >> 3) + y;
...
static inline uint64_t divu43(uint64_t n)
{
    uint64_t x = n;
    x = (x << 1) + x + 2;
    x = x - (x >> 7);
...
static inline uint64_t divu45(uint64_t n)
{
    uint64_t x = n + 1, y;
    y = (x << 2) + x;
    y = (y >> 3) + y;
...
static inline uint64_t divu47(uint64_t n)
{
    uint64_t x = n;
    x = ((x + 9) >> 1) + x + (x << 2) - (x >> 4)
        + (((((((x >> 4) + x) >> 1) + x) >> 3) + x) >> 7);
...
static inline uint64_t divu49(uint64_t n)
{
    uint64_t x = n;
#if LI
...
static inline uint64_t divu51(uint64_t n)
{
    uint64_t x = n;
    x = (x << 2) + x + 5;
    x = (x >> 8) + x;
...
static inline uint64_t divu53(uint64_t n)
{
    uint64_t x = n + 1, y;
#if LI
    y = x + (x >> 9);
...
static inline uint64_t divu55(uint64_t n)
{
    uint64_t x = n, y;
    y = x - (x >> 3);
    y = x - (y >> 4) + 148;
...
int checkall(void)
{
    for (int j = 0; j < sizeof tests / sizeof *tests; j++) {
        ...
        for (uint64_t i = 1; i < 0xffffffffffffffff; i = i + random()) {
            ...
            if (v != i / n) {
                fprintf(stderr, "Error @ 0x%16lx (%lu:%f) : expects %lu != has %lu\n", i, i, (float) i / n, i / n, v);
                break;
            }
        }
 ...
 * This is need to have a low variance between values.
 */
#define DIVSA(cst) \
    case cst : \
        for (int k = 0; k < loop; k++) { \
            for (uint64_t i = 0; i < 10000; i++) { \
                volatile uint64_t v = divu ## cst(i); \
            } \
        } \
        for (int k = 0; k < loop; k++) { \
